...
+ (NSString *)timeInfoWithDateString:(NSString *)dateString {
    ...
    if (time < 3600) { // 小于一小时
        ...
        return [NSString stringWithFormat:@"%.0f分钟%@", retTime,isFuture?@"后":@"前"];
    } else if (time < 3600 * 24) { // 小于一天，也就是今天
        retTime = time / 3600;
        retTime = retTime <= 0.0 ? 1.0 : retTime;
        return [NSString stringWithFormat:@"%.0f小时%@", retTime,isFuture?@"后":@"前"];
    ...
    // 第二个条件是隔年，对于隔年，只能是去年12月与今年1月这种情况
        ...
        return [NSString stringWithFormat:@"%d天%@", (abs)(retDay),isFuture?@"后":@"前"];
    } else  {
        if (abs(year) <= 1) {
            if (year == 0) { // 同年
                return [NSString stringWithFormat:@"%d个月%@", abs(month),isFuture?@"后":@"前"];
            }
            ...
            return [NSString stringWithFormat:@"%d个月%@", (abs)(12 - preMonth + month),isFuture?@"后":@"前"];
        }
        
        return [NSString stringWithFormat:@"%d年%@", abs(year),isFuture?@"后":@"前"];
    }
    
//    return (isFuture?@"1小时后":@"1小时前");
}
...
+(BOOL)isBetweenNow:(NSDate*)startDate endDate:(NSDate*)endDate
{
  if (startDate && endDate) {
    return (startDate.timeIntervalSinceNow <=0 && endDate.timeIntervalSinceNow >=0)||(startDate.timeIntervalSinceNow >=0 && endDate.timeIntervalSinceNow <=0);
  }
  return NO;
}
